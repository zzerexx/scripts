...
if not isfolder("UAIM") then
	makefolder("UAIM")
end
if isfile("UAIM.json") then
	writefile("UAIM\\Default.json", readfile("UAIM.json"))
	delfile("UAIM.json")
end
...
function save()
	...
	writefile("UAIM\\"..cfgname..".json", game:GetService("HttpService"):JSONEncode(a))
end
function load()
	local path = "UAIM\\"..selectedcfg
...
if isfile("UAIM\\Default.json") then
...
do -- Configs
	...
	Configs.Button({
		...
		Callback = function()
			if isfile("UAIM\\"..selectedcfg) then
				delfile("UAIM\\"..selectedcfg)
